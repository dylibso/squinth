...
// compute what the phase will be after a certain number of samples of the
// waveform have been computed at a particular sample rate
float ComputePhaseAfterNumSamples(int num_samples, float curr_phase,
                                  float sample_rate, float freq_hz) {
  ...
  // use the sample frequency against the wave frequency to determine how many
  // samples there are in one period of the wave
...
float *ComputeAudioSamplesMonophonic(
    ExtismPlugin *plugin, float sample_time, float freq_hz, float inputs[4],
    int32_t num_samples, // Only 256 is supported due to coordination with
                         // plugin modules
    float curr_phase,    // If you do not set this, the wavelen will start from
                         // beginning at the end of each period
    bool is_left_channel, int64_t sample, float sample_rate) {
  if (plugin == NULL) {
  ...
  ExtismArgs args = ExtismArgs{sample_time,
                               freq_hz,
                               curr_phase,
                               num_samples,
  ...
  int rc = extism_plugin_call(plugin, "batch_compute_wf",
                              (const uint8_t *)&args, sizeof(ExtismArgs));
